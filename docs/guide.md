...
The installer downloads a prebuilt app, verifies its checksum, adds an app launcher
entry, and opens OmaScripture. **No Rust toolchain or sudo required.** Supports
x86_64 Linux with glibc 2.39 or newer (including current Omarchy), a Wayland/X11
desktop, and OpenGL drivers.

On first launch, choose **WEB**, **KJV**, **NET**, or **NLT**, then click
**Start reading**. WEB and KJV download once for offline reading; NET and NLT
require internet access. Expand **More translations** for the offline catalog.
Setup finishes after your Bible loads and your choice is saved. If a download
fails, retry or choose another Bible. Study resources can be added later.

Change your default anytime in **Settings → Default Bible**. Existing users keep
their saved choice and skip onboarding. Run the same command to update; notes,
bookmarks, downloaded Bibles, and preferences are preserved.
...
Everything is installed in your user account:

| What | Where |
|---|---|
| App | `~/.local/bin/omascripture` |
| Launcher | `~/.local/share/applications/io.github.zachwilke.OmaScripture.desktop` |
| Uninstaller | `~/.local/share/omascripture/uninstall.sh` |

Uninstall with `bash ~/.local/share/omascripture/uninstall.sh`. Your study data
is kept unless you explicitly add `--purge`.

### Build from source / optional Omarchy integration

For other Linux architectures, or development, install Rust and the Wayland/X11,
xkbcommon, and OpenGL development libraries. On Omarchy, start with
`omarchy install dev-env rust`.

```bash
git clone --branch main https://github.com/zachwilke/omascripture ~/Projects/omascripture
cd ~/Projects/omascripture
./install.sh --launch
```

The source installer also adds an Omarchy Learn → Bible menu entry when Omarchy
is available. Pass `--no-menu` to skip it. Optional source installer flags:

| Flag | What it does |
|---|---|
| `--translation kjv` | Download a Bible before first launch |
| `--study basic` | Download core study packs (~60 MB) |
| `--study all` | Download every study pack (~200 MB) |
| `--study mhc,jfb,isbe` | Download selected study packs |
| `--bar` | Add a verse-of-the-day widget to the Omarchy bar |
| `--bind "SUPER + SHIFT + ALT + B"` | Add a Hyprland keybinding |

Nothing under `/usr/share/omarchy` is modified. CLI commands and `--tui` work
without a display. System Noto fonts are used when available, with bundled
fallback fonts.
